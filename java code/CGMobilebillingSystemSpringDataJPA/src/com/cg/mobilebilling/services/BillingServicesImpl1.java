...
@Component("billingServicesA")
public class BillingServicesImpl1 implements BillingServices {
	@Autowired
	private BillingDAOServicesC daoServicesCustomer;
	@Autowired
	private BillingDAOServicesA daoServicesPostAccount;
	@Autowired
	private BillingDAOServicesP daoServicesPlan;
	@Autowired
	private BillingDAOServicesB daoServicesBill;
	Customer customer;
	...
	public int generateMonthlyMobileBill(int customerID, long mobileNo, String billMonth, int noOfLocalSMS,
		...
		float billEstimate;
		if(daoServicesCustomer.findOne(customerID).getPostpaidAccount().get(mobileNo)!=null){
	...
	public List<PostpaidAccount> getCustomerAllPostpaidAccountsDetails(int customerID)
		...
		if(daoServicesCustomer.findOne(customerID)==null)
			throw new CustomerDetailsNotFoundException("CustomerID : "+customerID+" not found.");
		if(daoServicesCustomer.findOne(customerID)!=null){
		List<PostpaidAccount> list=new ArrayList<>(daoServicesCustomer.findOne(customerID).getPostpaidAccount().values());
		return list;
		}
		return null;
	}
	...
	public Bill getMobileBillDetails(int customerID, long mobileNo, String billMonth)
		...
		if(daoServicesPostAccount.findOne(mobileNo)==null)
			throw new PostpaidAccountNotFoundException("Invalid Mobile Number.");
		if(daoServicesPostAccount.findOne(mobileNo).getBill()==null)
		...
		if(!"JAN".equals(billMonth)&&!"FEB".equals(billMonth)&&
				...
				!"NOV".equals(billMonth)&&!"DEC".equals(billMonth))
			throw new InvalidBillMonthException("Enter Correct Month. (Format 'JAN','FEB' and so on!)");
	bill=(Bill) daoServicesCustomer.findOne(customerID).getPostpaidAccount().get(mobileNo).getBill();
	System.out.println(bill.getBillID());
		//	daoServicesBill.findOne(bi)
		return (Bill) daoServicesPostAccount.findOne(mobileNo).getBill();
	}
	...
	public List<Bill> getCustomerPostPaidAccountAllBillDetails(int customerID, long mobileNo)
		...
		List<Bill> list=new ArrayList<>(postPaidAccount.getBill().values());
		return list;
		}
		return null;
	}
	...
	public boolean changePlan(int customerID, long mobileNo, int planID) throws CustomerDetailsNotFoundException,
		...
		if(daoServicesPlan.findOne(planID)==null)
			throw new PlanDetailsNotFoundException("No such plan found.");
		customer=daoServicesCustomer.findOne(customerID);
		if(customer.getPostpaidAccount().get(mobileNo)!=null){
			plan=daoServicesPlan.findOne(planID);
			postPaidAccount=new PostpaidAccount(plan);
			customer.getPostpaidAccount().get(mobileNo).setPlan(plan);
			postPaidAccount.setCustomer(customer);
	...
	public Plan getCustomerPostPaidAccountPlanDetails(int customerID, long mobileNo)
		...
		if(daoServicesCustomer.findOne(customerID).getPostpaidAccount().get(mobileNo)!=null)
		return daoServicesCustomer.findOne(customerID).getPostpaidAccount().get(mobileNo).getPlan();
		return null;
	}

	@Override
	public int createPlan(int monthlyRental, int freeLocalCall, int freeStdCall, int freeLocalSMS, int freeStdSMS,
		...
		plan=daoServicesPlan.save(new Plan(monthlyRental, freeLocalCall, freeStdCall, freeLocalSMS, freeStdSMS, freeInternetDataUsageUnits, localCallRate, stdCallRate, localSMSRate, stdSMSRate, internetDataUsageRate, planCircle, planName));
