...
public class PayrollServicesImpl implements PayrollServices {
	...
	public float calculateNetSalary(int associateId) throws AssociateDetailsNotFoundExceptions,PayrollServicesDownExceptions{
		...
		if(associate!=null){
			...
			associate.getSalary().setGrossSalary((float)(associate.getSalary().getOtherAllowance()+associate.getSalary().getConveyenceAllowance()+associate.getSalary().getPersonalAllowance()+associate.getSalary().getHra()));
			associate.getSalary().setNetSalary((float)associate.getSalary().getGrossSalary()-100-associate.getSalary().getCompanyPf()-associate.getSalary().getEpf());
			float annualSalary = (float) (12*(associate.getSalary().getGrossSalary()));
			float sum = ((float)(associate.getSalary().getCompanyPf()+associate.getSalary().getEpf()+associate.getYearlyInvestmentUnder80C()));
			float yearlyTax = 0;
			if(sum>150000)
				sum=150000;
			...
			if(annualSalary>250000 && annualSalary<500000){
				if((annualSalary-250000-sum)>150000)
					yearlyTax=0;
				else
	...
	@Override
	public Associate[] getAllAssociateDetails() throws PayrollServicesDownExceptions{
